...
echo <<< END
<form action="kontakt_edytuj.php" method="post">
<input type="hidden" name="kat" value="$kat"/>
Jaki kontakt chcesz edytować? :<br/>
...
END;
	...
	{
		$sql = "SELECT * FROM kontakt WHERE Kat='$kat' ORDER BY Waga";
		if($rezultat = @$polaczenie->query($sql))
		{
			$ile_postow = $rezultat->num_rows;
			for($i=0;$i<$ile_postow;$i++)
			{
				$sql2 = "SELECT * FROM Kontakt WHERE Kat='$kat' ORDER BY Waga LIMIT 1 OFFSET $i";
...
echo '</select>&nbsp;&nbsp;<input type="submit" value="EDYTUJ"></input></form><br/><br/>';	
